...
pdf("C:/Advancement/g1.pdf", 4, 4)
g.0 = dnorm(x=y, mean=y.t.minus.1, sd=sigma)
r.0 = -1*dnorm(x=y, mean=zeta(y.t.minus.1), sd=sigma)
g.1 = g.0 + r.0

plot(y, g.0, ylab = "", xlab = "", type = "l", col = "red", lwd = 2, ylim = c(min(c(g.0, r.0, g.1)), max(c(g.0,r.0,g.1))),
     xaxt = "n", yaxt = "n")
abline(v=c(a.t, b.t), lwd = 2, lty = "dashed")
abline(h=0, lwd = 1)

lines(y, r.0, lwd = 2, col = "green")
lines(y, g.1, lwd = 2, col = "black")
mtext(text = c(expression(a[t]), expression(b[t])), side = 1, at = c(a.t, b.t))
#legend(-3,0.4, legend = c("g_0(y,t)","r_0(y,t)", "g_0(y,t) + r_0(y,t)"), lty = "solid", lwd = 2, col = c("red", "green", "black"))
dev.off()

pdf("C:/Advancement/g2.pdf", 4, 4)
...
plot(y, g.1, ylim = c(min(c(g.0, r.0, g.1)), max(c(g.0,r.0,g.1))), #ylim = c( min(c(g.1, r.1, g.2)), max(c(g.1, r.1, g.2)) ), 
        ylab = "",
        xlab = "",
        type = "l",
        col = "red",
...
mtext(text = c(expression(a[t]), expression(b[t])), side = 1, at = c(a.t, b.t))
#legend(-3,0.2, legend = c("g_1(y,t)","r_1(y,t)", "g_1(y,t) + r_1(y,t)"), lty = "solid", lwd = 2, col = c("red", "green", "black"))

dev.off()

pdf("C:/Advancement/g3.pdf", 4,4)
...
plot(y, g.3, ylim = c(min(c(g.0, r.0, g.1)), max(c(g.0,r.0,g.1))),
     type = "l", 
     lwd = 2, 
     col = "black", 
     ylab = "", 
